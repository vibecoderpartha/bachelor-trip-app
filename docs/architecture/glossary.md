...
report. As an Accepted governance source, later documents must use these terms
or explicitly propose an amendment.
...
- A term defined here does not by itself set an ADR's status. Permanent
...
| **Participant** | A person acting within a Group whose identity in group-owned records is represented by a stable Group Member identity. | Group-owned events, expenses, settlements, Todos, and related target records. | A display name, arbitrary text label, global Profile, or unauthenticated legacy persona. | A Participant is the domain use of a Group Member identity. [ADR-0004](decisions/ADR-0004-group-member-id-is-participant-identity.md) locks the permanent `group_members.id` reference rule. |
...
| **Authentication** | Establishing and maintaining which Auth User is making a request. | Identity and session security; target state. | Authorization to access a Group or any group-owned record. | Supabase Auth is the authoritative identity provider under [ADR-0002](decisions/ADR-0002-supabase-auth-is-authoritative.md); authorization additionally requires group-scoped checks. |
...
| **Feature parity** | Verifiable preservation of an existing v1 user-visible behaviour or necessary supporting behaviour in the target conversion, adjusted only as required for secure tenant scoping and approved configuration. | Migration and acceptance. | Pixel identity, preservation of a defect by default, new feature development, or an undocumented omission. | The Accepted [feature-parity contract](feature-parity-test-contract.md) enumerates the required evidence. Differences require an Intentional parity exception or an architecture/security requirement. |
